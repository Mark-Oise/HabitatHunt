    ...
    tabGroups.forEach(tabGroup => {
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                // Get the specific tab group this tab belongs to
                const groupId = tab.closest('[data-tabs]').getAttribute('data-tabs');
                ...
                tabPanels.forEach(panel => {
                    if (panel.id.includes(groupId.split('-')[1])) {  // Only affect panels for this lead
                ...
                if (targetPanel) {
                    targetPanel.classList.remove('hidden', 'opacity-0');
                }
            });
        });
    });
